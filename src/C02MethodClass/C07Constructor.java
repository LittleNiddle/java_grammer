...
//        java자료구조 내에 직접 만든 객체를 담아 관리
        ...
        System.out.println(myList); // toString , toString
    }
}
...
//    생성자특징 : 1)클래스명과 메서드명이 동일 2)반환타입이(void X) 없음
//    생성자의 사용 목적 : 객체 생성 시점에 객체변수값들을 초기화(세팅) + setter의 사용 지양
    public Calender(String year, String month, String day){
        this.year = year;
        this.month = month;
        this.day = day;
    }
//    직접 생성자를 1개라도 추가할 경우, 초기(기본)생성자는 무시가 되므로, 필요시 별도로 추가. 변수의 안정성을 위해 setter의 사용을 지양하고, 생성자를 사용.
    public Calender(){
